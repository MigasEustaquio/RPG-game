people = {

            'Seashore' : {
              'Kairi' : {
                0: {'speech' : 'Kairi: Sora, you lazy bum. I knew that I\'d find you snoozing down here.',
                      'reward' : 'no',
                      'present' : 'yes',
                },
                1: {'present' : 'no'}
              },
            },


            'Cove' : {
              'Kairi' : {
                1: {'speech' : 'Kairi: Sora, tomorrow we set sail.',
                      'reward' : 'no',
                      'present' : 'yes',
                },
              },

              'Riku' : {
                0: {'speech' : 'Riku: Come on Sora, I thoght you were stronger than that!.',
                      'reward' : 'no',
                      'present' : 'yes',
                },
              },
            },


            'Green Room' : {
              'Leon' : {
                  0: {'speech' : 'Leon: Here\'s the map of the town!',
                        'reward' : 'mapUpdate',
                        'mapUpdate' : '1',
                        'present' : 'yes',
                  },
              },
            },

            'Red Room' : {
              'Yuffie' : {
                  0: {'speech' : 'Yuffie: Hi there, Sora!\nDid you know that there\'s a shop in the Fisrt District?\n Try typing \'enter shop\'',
                          'reward' : 'key item',
                          'key item' : 'First District Shop location',
                          'present' : 'yes',
                  },
              },
            },
            'First District Shop' :{
              'Moogle' : {
                      0: {'speech' : 'Moogle: Hi there, Kupo!\nTo buy an item just type \'buy [item]\'\n\nHere\'s what I have in stock:',
                          'reward' : 'no',
                          'present' : 'yes',
                      },
              },
            },

            '2nd Floor' : {
              'Roxas' : {
                      0: {'speech' : 'Roxas: Story is now 1!',
                        'reward' : 'story',
                        'story' : 1,
                        'present' : 'yes',
                      },
                      2: {'speech' : 'Roxas: Here is an elixir!',
                        'reward' : 'item',
                        'item' : 'elixir',
                        'present' : 'yes',
                      },
                      3: {'speech' : 'Roxas: ',
                        'reward' : 'item',
                        'item' : 'elixir',
                        'present' : 'no',
                      },
              },

              'Axel' : {
                      0: {'speech' : 'Axel: Here is a pot!',
                        'reward' : 'keyblade',
                        'keyblade' : 'Kingdom Key D',
                        'present' : 'yes',
                      },
                      1: {'speech' : 'Axel: Here is an ether!',
                        'reward' : 'item',
                        'item' : 'ether',
                        'present' : 'no',
                      },
                      2: {'speech' : 'Axel: Here is another ether!',
                        'reward' : 'item',
                        'item' : 'ether',
                        'present' : 'yes',
                      },
              },
            },

}